...
test_time=int(argv[1])		#-1 Run for infinite time
...
while True if test_time==-1 else (time.time()-t0)/60<test_time:
	try:
		...
		s6=open(file_wetness, "r")
		wetness=s6.readlines()
		s6.close()
		while wetness and any(x not in '0123456789.' for x in wetness[-1][:-1]):
					wetness=wetness[:-1]
		...
		if humidity:
			...
			s5.close()
		if wetness:
			s6=open(file_wetness,"w")
			s6.write(str(wetness[-1][:-1])+"\n")
			s6.close()
		...
		(not humidity or abs(float(humidity[-1][:-1])-float(humidity[0][:-1]))<min_humidity_difference) and\
		(not wetness or abs(float(wetness[-1][:-1])-float(wetness[0][:-1]))<min_wetness_difference):        #slicing for excluding trailing \n
			continue
		...
		if wetness:
			mydict['Wetness']=float(wetness[-1][:-1])
